...
function buildDailyForecast(location, forecast, twilight, units) {
    ...
    for (let i = 1; i < 8; i++) {
        ...
        for (let j = 0; j < forecast.length; j++) {
            if (
                moment(forecast[j].temperature.validTime).format('YYYY-MM-DD') == moment(sunrise).format('YYYY-MM-DD') &&
        ...
        for (let j = 0; j < forecast.length; j++) {
            if (
                moment(forecast[j].weather.validTime).format('YYYY-MM-DD') == moment(sunrise).format('YYYY-MM-DD') &&
...
function getSubForecast(timeOfDay, precipitation, skyCover, weather, index) {
    ...
    else if (skyCover > 0 && skyCover < 26 && weather_type == null && intensity == null) {
        subForecast = {
    ...
    else if (skyCover > 25 && skyCover < 51 && weather_type == null && intensity == null) {
        subForecast = {
    ...
    else if (skyCover > 50 && skyCover < 76 && weather_type == null && intensity == null) {
        subForecast = {
    ...
    // '21000': 'Light Fog',
    else if (coverage == 'Patchy' && weather_type == 'fog' && intensity == null) {
        subForecast = {
    ...
    // '40000': 'Drizzle',
    // '42000': 'Light Rain',
    else if ((weather_type == 'rain' || weather_type == 'rain_showers') && intensity == 'light') {
        subForecast = {
    ...
    // '40010': 'Rain',
    else if ((weather_type == 'rain' || weather_type == 'rain_showers') && intensity == 'moderate') {
        subForecast = {
            'shortForecast': 'Rain',
    ...
    // '50000': 'Snow',
    else if (coverage == 'Likely' && weather_type == 'snow' || (weather_type == 'snow_showers') && intensity == 'moderate') {
        subForecast = {
            'shortForecast': 'Snow',
    ...
    if (subForecast.shortForecast == 'Unknown') {
        console.log('IWO:Warn', index, 'Precipitation:', precipitation, 'Sky Cover:', skyCover, 'Weather', weather)
    }
    return subForecast
}
