...
function convertValidTimeValues(values, timeZone) {
  ...
  for (let a = 0; a < values.length; a++) {
    ...
    for (let i = 0; i < numberHours; i++) {
      const newTime = moment(parts[0]).add(i, "hours").tz(timeZone).format();
...
function processGridData(gridData, location) {
  ...
  const apparentTemperature = convertValidTimeValues(gridData.apparentTemperature.values, timeZone)
  const probabilityOfPrecipitation = convertValidTimeValues(gridData.probabilityOfPrecipitation.values, timeZone)
  const skyCover = convertValidTimeValues(gridData.skyCover.values, timeZone)
  const temperature = convertValidTimeValues(gridData.temperature.values, timeZone)
  const weather = convertValidTimeValues(gridData.weather.values, timeZone)
  const windDirection = convertValidTimeValues(gridData.windDirection.values, timeZone)
  const windGust = convertValidTimeValues(gridData.windGust.values, timeZone)
  const windSpeed = convertValidTimeValues(gridData.windSpeed.values, timeZone)
