...
async function getTwilight(location) {
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (results.status == 200) {
        const sunrise = moment(results.data.results.sunrise).tz(location.timeZone.zoneName).format();
        const sunset = moment(results.data.results.sunset).tz(location.timeZone.zoneName).format();
