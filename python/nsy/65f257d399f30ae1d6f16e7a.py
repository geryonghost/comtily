...
while not empty:
    ...
    driver = notscrapyet.selenium_get(driver,inventory_url) # Scrolls through page to display ALL listings

    if driver.title:
        # posting_total = int(driver.find_element(notscrapyet.By.CLASS_NAME,'')) #.get_attribute('innerHTML'))
        posting_total = driver.find_element(notscrapyet.By.XPATH("//span[contains(@class,'header-value')]"))
        print(posting_total)
